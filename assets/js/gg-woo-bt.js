    ...
    (function ($) {
      var GG_Woo_BT = {
        ...
        hookFoundVariation: function hookFoundVariation() {
          $(document).on('found_variation', function (e, t) {
            ...
            if ($product.length) {
              ...
              $product.find('.gg_woo_bt-price-new').html(GG_Woo_BT.priceHtml(t['display_price'], new_price)).show();
              ...
              if (t['image']['url'] && t['image']['srcset']) {
                ...
                $product.find('.gg_woo_bt-thumb-new').html('<img src="' + t['image']['url'] + '" srcset="' + t['image']['srcset'] + '"/>').show();
              }
        ...
        isReady: function isReady($wrap) {
          ...
          if (gg_woo_bt_params.position === 'before_add_to_cart' && $products.attr('data-product-type') === 'variable' && $products.attr('data-variables') === 'no') {
          ...
          if (is_selection) {
            ...
            $notice.html(gg_woo_bt_params.text.variation_notice.replace('%s', '<strong>' + selection_name + '</strong>')).slideDown();
          } else {
        ...
        calcPrice: function calcPrice($wrap) {
          ...
          $products.find('.gg_woo_bt-product-together').each(function () {
            ...
            if (_qty > 0 && _id > 0) {
              ...
              if (show_price === 'total') {
                ...
                $el.find('.gg_woo_bt-price-new').html(GG_Woo_BT.priceHtml(origin_total, _total) + _price_suffix).show();
              }
          ...
          if ($product_this.length) {
            ...
            if (total > 0) {
              ...
              $product_this.find('.gg_woo_bt-price-new').html(GG_Woo_BT.priceHtml(origin_total, _total) + _price_suffix).show();
            } else {
          ...
          if (count > 0) {
            ...
            $total.html(gg_woo_bt_params.text.additional_price + ' ' + total_html + ori_price_suffix).slideDown();
        ...
        priceHtml: function priceHtml(regular_price, sale_price) {
          ...
          if (sale_price < regular_price) {
            price_html = '<del>' + GG_Woo_BT.formatPrice(regular_price) + '</del> <ins>' + GG_Woo_BT.formatPrice(sale_price) + '</ins>';
          } else {
        ...
        formatPrice: function formatPrice(total) {
          ...
          var total_formatted = GG_Woo_BT.formatMoney(total, gg_woo_bt_params.price_decimals, '', gg_woo_bt_params.price_thousand_separator, gg_woo_bt_params.price_decimal_separator);

          switch (gg_woo_bt_params.price_format) {
            case '%1$s%2$s':
              total_html += '<span class="woocommerce-Price-currencySymbol">' + gg_woo_bt_params.currency_symbol + '</span>' + total_formatted;
              break;
            ...
            case '%2$s%1$s':
              total_html += total_formatted + '<span class="woocommerce-Price-currencySymbol">' + gg_woo_bt_params.currency_symbol + '</span>';
              break;
...
//# sourceMappingURL=gg-woo-bt.js.map
