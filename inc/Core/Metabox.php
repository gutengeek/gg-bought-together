...
abstract class Metabox {
	...
	public function __construct() {
		$this->metabox_id    = 'gg_woo_bt-metabox-form-data';
		$this->metabox_label = esc_html__( 'Options', 'gg-woo-bt' );

		add_action( 'save_post_gg_woo_bts', [ $this, 'save' ], 10, 2 );
	}
			...
			foreach ( $form_meta_keys as $form_meta_key ) {
				...
				if ( 'taxonomy_select' === $setting_field['type'] ) {
					if ( isset( $setting_field['multiple'] ) && $setting_field['multiple'] ) {
						$_POST[ $form_meta_key ] = isset( $_POST[ $form_meta_key ] ) ? gg_woo_bt_clean( $_POST[ $form_meta_key ] ) : [];
					} else {
						$_POST[ $form_meta_key ] = isset( $_POST[ $form_meta_key ] ) ? gg_woo_bt_clean( $_POST[ $form_meta_key ] ) : '';
					}
				}
				...
				if ( isset( $_POST[ $form_meta_key ] ) ) {
					if ( ! empty( $setting_field['type'] ) ) {
						...
						$form_meta_value = apply_filters(
							'gg_woo_bt_pre_save_form_meta_value',
	...
	public function save_settings_options( $options, $option_name ) {
		...
		if ( $update_options ) {
			...
			do_action( 'gg_woo_bt_after_save_settings', $update_options, $old_options );
		}
	}
