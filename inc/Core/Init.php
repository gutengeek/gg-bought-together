...
use GG_Woo_BT\Admin\Admin;
use GG_Woo_BT\Admin\Metabox as Metabox;
use GG_Woo_BT\Common\Interfaces\Intergration;
use GG_Woo_BT\Frontend\Enqueue;
...
class Init {
	...
	public function __construct() {
		...
		$this->plugin_name        = GGWOOBT;
		$this->version            = GGWOOBT_VERSION;
		$this->plugin_basename    = GGWOOBT_PLUGIN_BASENAME;
		$this->plugin_text_domain = GGWOOBT_PLUGIN_TEXT_DOMAIN;
	...
	public function define_intergrations() {
		...
		foreach ( $intergrations as $intergration ) {
			$class  = "GG_Woo_BT\\Common\\Integrations\\" . $intergration;
			$object = new   $class();
	...
	private function define_global_init() {
		...
		$settings          = get_option( 'gg_woo_bt_settings' );
		$gg_woo_bt_options = apply_filters( 'gg_woo_bt_get_settings', $settings );
	}
	...
	public function define_controllers() {
		$controllers = [
			...
			'cart'    => 'Cart@register_hook_callbacks',
			'order'   => 'Order@register_hook_callbacks',
		];
	...
	public function load_controller( $call ) {
		...
		$controller = "\GG_Woo_BT\Frontend\Controller\\" . $controller;
	...
	public function load_admin_controller( $call ) {
		@list( $controller, $action ) = explode( '@', $call );
		$controller = "\GG_Woo_BT\Admin\Controller\\" . $controller;
		$object     = $controller::get_instance();
		$object->{$action}();
	}
	...
	private function define_admin_hooks() {
		...
		$controllers = [
			'search'  => 'Search@register_hook_callbacks',
			'product' => 'Product@register_hook_callbacks',
		];
