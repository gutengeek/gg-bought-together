 ...
 * @since       1.0
 */
namespace GG_Woo_BT\Libraries\Form;

use GG_Woo_BT\Libraries\Form\Helper;
use GG_Woo_BT\Libraries\Form\Field\File;
use GG_Woo_BT\Libraries\Form\Field\Iconpicker;
use GG_Woo_BT\Libraries\Form\Field\Map;
use GG_Woo_BT\Libraries\Form\Field\Uploader;
use GG_Woo_BT\Libraries\Form\Field\Taxonomy;
use GG_Woo_BT\Libraries\Form\Field\Page;
...
class Form {
	...
	public function __construct() {
		// Ajax.
		add_action( 'wp_ajax_gg_woo_bt_search_users', [ '\GG_Woo_BT\Libraries\Form\Helper', 'ajax_search_users' ] );
	...
	public function render_field( $field ) {
		...
		if ( ! isset( $field['id'] ) || ! isset( $field['type'] ) ) {
			return sprintf( esc_html__( 'The field ID or field type is required.', 'gg-woo-bt' ), $field ['type'] );
		}
		...
		switch ( $field['type'] ) {
			...
			case 'html':
				...
				break;
			default:
				do_action( 'gg_woo_bt_form_render_field_' . $field['type'], $field, $this );
				//return sprintf( esc_html__( 'The field type: %s does not exist!', 'gg-woo-bt' ), $field ['type'] );
				break;
		}
	}
	...
	public function ajax_user_search( $args = [] ) {
		...
		$defaults = [
			...
			'placeholder'  => esc_html__( 'Enter username', 'gg-woo-bt' ),
			'label'        => null,
			'desc'         => null,
			'class'        => '',
			'disabled'     => false,
		...
		$args['class'] = 'gg_woo_bt-ajax-user-search ' . $args['class'];

		$output = '<span class="gg_woo_bt_user_search_wrap">';
		$output .= $this->text_field( $args );
		$output .= '<span class="gg_woo_bt_user_search_results hidden"><a class="gg_woo_bt-ajax-user-cancel" aria-label="' . esc_html__( 'Cancel',
				'gg-woo-bt' ) . '" href="#">x</a><span></span></span>';
		$output .= '</span>';
	...
	public function text_field( $args ) {
		switch ( $args['type'] ) {
			case 'text_small' :
				$class        = 'gg_woo_bt-text-small';
				$args['type'] = 'text';
				break;
			case 'text_medium' :
				$class        = 'gg_woo_bt-text-medium';
				$args['type'] = 'text';
				break;
			case 'text_number' :
				$class        = 'gg_woo_bt-text-small gg_woo_bt-text-number';
				$args['type'] = 'number';
				break;
			case 'text_url' :
				$class        = 'gg_woo_bt-text-url';
				$args['type'] = 'url';
				break;
			case 'text_email' :
				$class        = 'gg_woo_bt-text-email';
				$args['type'] = 'email';
				break;
			case 'text_tel' :
				$class        = 'gg_woo_bt-text-tel';
				$args['type'] = 'tel';
				break;
			case 'password' :
				$class        = 'gg_woo_bt-text-password';
				$args['type'] = 'password';
				break;
			case 'hidden' :
				$class        = 'gg_woo_bt-text-hidden';
				$args['type'] = 'hidden';
				break;
			default :
				$class        = '';
		...
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'gg_woo_bt-text ' . $class . ' regular-text form-control';
		} elseif ( ! strpos( $args['class'], 'gg_woo_bt-text' ) ) {
			$args['class'] .= ' gg_woo_bt-text ' . $class . ' regular-text form-control';
		}
	...
	public function colorpicker_field( $args ) {
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'gg_woo_bt-colorpicker form-control';
		} elseif ( ! strpos( $args['class'], 'gg_woo_bt-colorpicker' ) ) {
			$args['class'] .= ' gg_woo_bt-colorpicker form-control';
		}
	...
	public function output_normal( $fields ) {
		...
		if ( function_exists( 'wp_unique_id' ) ) {
			$form_id = wp_unique_id( 'gg_woo_bt-form-' );
		} else {
			$form_id = 'gg_woo_bt-form-' . (string) ++$id_counter;
		}

		echo '<div class="js-gg_woo_bt-metabox-wrap">';
	...
	public function get_repeater_field_value( $field, $field_group, $fields ) {
		 ...
		 */
		$field_value = apply_filters( "gg_woo_bt_get_repeater_field_{$field['id']}_value", $field_value, $field, $field_group, $fields );
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( 'gg_woo_bt_get_repeater_field_value', $field_value, $field, $field_group, $fields );
	...
	public function get_repeater_field_id_value( $field, $field_group, $fields ) {
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( "gg_woo_bt_get_repeater_field_{$field['id']}_id_value", $field_value, $field, $field_group, $fields );
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( 'gg_woo_bt_get_repeater_field_id_value', $field_value, $field, $field_group, $fields );
	...
	public function get_repeater_field_id( $field, $fields, $default = false ) {
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( "gg_woo_bt_get_repeater_field_{$field['id']}_id", $field_id, $field, $fields, $default );
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( 'gg_woo_bt_get_repeater_field_id', $field_id, $field, $fields, $default );
	...
	public function get_repeater_field_hidden_id( $field, $fields, $default = false ) {
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( "gg_woo_bt_get_repeater_field_{$field['id']}_hidden_id", $field_id, $field, $fields, $default );
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( 'gg_woo_bt_get_repeater_field_hidden_id', $field_id, $field, $fields, $default );
