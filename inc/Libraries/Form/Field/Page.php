...
class Page {
	...
	public function render_page_select() {
		...
		$output = '<div class="gg_woo_bt-field-wrap gg_woo_bt-page-select-wrap form-group" id="' . sanitize_key( $this->form->form_id . $args['id'] ) . '-wrap" >';
	...
	protected function loop_terms( $all_terms ) {
		...
		$options .= sprintf( '<option value="%1$s" %2$s>%3$s</option>',
				"",
				$selected,
				esc_html__( '-- Select Page --', 'gg-woo-bt' )
		);
